...
public class Comet extends JavaPlugin implements Listener, VelinquishPlugin {
	...
	public void onEnable() {
		...
		Common.registerCommand(new CommandManager(getConfig().getString("main-command"), this));
	}
	...
	private void loadPaths() {
		for (String path : locations.getConfigurationSection("Paths").getKeys(false))
			locationManager.addPath(loc(path));
	}

	private Path loc(String path) {
		Location spawn = new Location(Bukkit.getWorld(locations.getString("Paths." + path + ".world")), locations.getDouble("Paths." + path + ".spawn.x"),
				locations.getDouble("Paths." + path + ".spawn.y"),
				locations.getDouble("Paths." + path + ".spawn.z"));
		Location desti = new Location(Bukkit.getWorld(locations.getString("Paths." + path + ".world")), locations.getDouble("Paths." + path + ".destination.x"),
				locations.getDouble("Paths." + path + ".destination.y"),
				locations.getDouble("Paths." + path + ".destination.z"));
		return new Path(spawn, desti);
	}
	...
	public void addBrokenMeteor(Entity entity, Location loc) {
		...
		debug("The comet broke! Spawning a falling block in a safer place");
		...
		//	if (!block.isValid())
		//		addBrokenMeteor(block, loc); //maybe dangerous? It can spawn where original block broke.
		//	else
		//		block.setVelocity(new Vector(0,0,0));
	...
	public void onRightClickMeteor(PlayerInteractEvent e) {
		if (e.getClickedBlock() != null &&
				e.getClickedBlock().getType().equals(Material.ENDER_STONE) &&
	...
	public void onMeteorLand(EntityChangeBlockEvent e) {
		...
		checks.get(e.getEntity()).cancel(); //This isn't working? The break will constantly create a new entity on replace
